...
os.environ["NEBULA_API_KEY"] = (
    "key_L2bhhSmBCKX-wh-c_QcI8g==.mouoseeWHgXvrMcc7pYDYAKMhtXljXfwMYXblotgI84="
)
...
def test_memory_storage():
    ...
    collection = client.create_collection(
        name=unique_name, description="Test collection for individual memories"
    )
...
def test_conversation_storage():
    ...
    collection = client.create_collection(
        name=unique_name, description="Test collection for conversation memories"
    )
...
def test_retrieval():
    ...
    collection = client.create_collection(
        name=unique_name, description="Test collection for retrieval"
    )
    ...
    for i, result in enumerate(results):
        print(
            f"   Result {i + 1}: {result.content[:80]}... (score: {result.score:.3f})"
        )
...
def test_chat():
    ...
    collection = client.create_collection(
        name=unique_name, description="Test collection for chat"
    )
    ...
    for content in context_memories:
        client.store(
            agent_id="test_chat_agent", content=content, collection_id=collection.id
        )
...
def test_search():
    ...
    collection = client.create_collection(
        name=unique_name, description="Test collection for search"
    )
    ...
    for content in diverse_content:
        client.store(
            agent_id="test_search_agent", content=content, collection_id=collection.id
        )
...
def test_collection_memories():
    ...
    collection = client.create_collection(
        name=unique_name, description="Test collection for memory retrieval"
    )
    ...
    print(
        f"✅ Retrieved {len(memories)} memories from collection using documents endpoint"
    )
    for i, memory in enumerate(memories):
