...
def test_document_ingestion(client, collection_id):
    ...
    try:
        ...
        if resp.status_code in (200, 202):
            ...
            for i in range(10):
                try:
                    status_resp = httpx.get(f"{client.base_url}/v1/ingestions/{ingestion_id}", headers=headers)
...
def test_memory_operations(client, collection_id):
    ...
    if collection_id:
        try:
            memories = client.get_cluster_memories(collection_id=collection_id, limit=10)
...
def test_search_operations(client, collection_id):
    ...
    try:
        ...
        for i, result in enumerate(results[:3]):
            print(f"   {i+1}. Score: {result.score}, Content: {result.content[:50]}...")
    except Exception as e:
